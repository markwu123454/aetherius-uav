...
export function shouldDisplayLog(
    ...
    // Category filter: default include all except EA and ER
    const excludedCats = ['EA', 'ER'];
    if (options.categories) {
        if (!options.categories.includes(cat)) return false;
    } else {
        if (excludedCats.includes(cat)) return false;
    }
 ...
 */
export function getError(entries: LogEntry[]): LogEntry[] {
    const seenER = new Set<string>();
    const eaMap = new Map<string, LogEntry>();

    for (const entry of entries) {
        const idStr = String(entry.log_id);
        if (idStr.startsWith('ER')) {
            seenER.add(idStr.slice(-4));
        } else if (idStr.startsWith('EA')) {
            const suffix = idStr.slice(-4);
            eaMap.set(suffix, entry);
        }
    }

    return Array.from(eaMap.entries())
        .filter(([suffix]) => !seenER.has(suffix))
        .map(([_, entry]) => entry);
}
